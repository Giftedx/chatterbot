...
export class MultimodalIntegrationService {
  ...
  public async generateContentRecommendations(
    ...
    try {
      ...
      const qualityRecs = await this.suggestQualityImprovements(recentFiles);
      recommendations.push(...qualityRecs);

      // Workflow optimization recommendations
      const workflowRecs = await this.suggestWorkflowOptimizations(recentFiles);
      recommendations.push(...workflowRecs);

      // Related content recommendations
      const relatedRecs = await this.suggestRelatedContent(recentFiles, userId);
