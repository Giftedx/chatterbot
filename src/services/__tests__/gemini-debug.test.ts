...
jest.mock('@google/generative-ai', () => {
  ...
  return {
    GoogleGenerativeAI: jest.fn(() => {
      ...
      return {
        getGenerativeModel: jest.fn((options: any) => {
          console.log('getGenerativeModel called with:', options);
          return {
            startChat: jest.fn((chatOptions: any) => {
              console.log('startChat called with:', chatOptions);
              return {
                sendMessage: jest.fn().mockImplementation(async (prompt: string) => {
...
describe('Gemini Debug Test', () => {
  ...
  it('should debug the full call flow', async () => {
    ...
    const geminiService = new GeminiService(mockCacheService as unknown as CacheService);
